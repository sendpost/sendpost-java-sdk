...
public class EIP {
  ...
  public EIP publicIP(@javax.annotation.Nonnull String publicIP) {
  ...
  public boolean equals(Object o) {
    ...
    EIP EIP = (EIP) o;
    return Objects.equals(this.publicIP, EIP.publicIP);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to EIP
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!EIP.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in EIP is not found in the empty JSON string", EIP.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!EIP.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `EIP` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : EIP.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!EIP.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'EIP' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<EIP> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(EIP.class));

       return (TypeAdapter<T>) new TypeAdapter<EIP>() {
           @Override
           public void write(JsonWriter out, EIP value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public EIP read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of EIP given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of EIP
   * @throws IOException if the JSON string is invalid with respect to EIP
   */
  public static EIP fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, EIP.class);
  }

  /**
   * Convert an instance of EIP to an JSON string
   *
   * @return JSON string
   */
